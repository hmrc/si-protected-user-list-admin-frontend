...
import models.{IdentityProviderId, TaxIdentifier}
...
final case class Insert(
  ...
  def toRequestJSON(stridePID: String): JsValue = Json.obj(
  ...
  private def getTaxID =
    optNINO.map(TaxIdentifier(NINO, _)) orElse
      optSAUTR.map(TaxIdentifier(SAUTR, _))
}
object Insert {
